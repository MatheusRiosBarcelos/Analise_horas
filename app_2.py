...
ordem['delta_time_min'] = ordem['delta_time_hours']*60
soma_por_estacao = ordem.groupby('estacao')['delta_time_min'].sum().reset_index().round(2)
soma_por_estacao.rename(columns={'delta_time_min': 'Tempo de uso (min)'}, inplace=True)
soma_por_estacao.rename(columns={'estacao': 'Estação de Trabalho'}, inplace=True)
total_de_horas_pedido = round(ordem['delta_time_min'].sum(),2)

fig = px.pie(soma_por_estacao, values='Tempo de uso (min)', names='Estação de Trabalho', title='Proporção de Tempo de Uso por Máquina em Cada Pedido', width=800, height=500)
...
nova_linha = {'Estação de Trabalho': 'Total', 'Tempo de uso (min)': total_de_horas_pedido}
